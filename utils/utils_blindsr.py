...
if __name__ == '__main__':
    ...
    for i in os.listdir(path):
        print(path+i)
        img = util.imread_uint(str(path + i), 3)
    ...
    #result = np.zeros([72*20,72*20,3])
    #for i in range(3):
        img_lq, img_hq = degradation_bsrgan(img, sf=sf, lq_patchsize=lq_)
    #print(i)
    ...
    #result[:, i * 72:(i + 1) * 72, :] = img_concat
        util.imsave(lq_nearest, i[:-4]+'_lr.png')
        util.imsave(util.single2uint(img_hq), i[:-4] + '_sr.png')
        #util.imsave(img_concat, str(0)+'.png')
